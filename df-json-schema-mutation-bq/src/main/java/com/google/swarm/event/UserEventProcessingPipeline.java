...
public class UserEventProcessingPipeline {
  ...
  public static void main(String[] args) {
    UserEventProcessingPipelineOptions options =
  ...
  public static PipelineResult run(UserEventProcessingPipelineOptions options) {
    ...
    CloudBigtableScanConfiguration bigTableConfig =
        new CloudBigtableScanConfiguration.Builder()
            ...
            .build();
    FailsafeElementCoder<PubsubMessage, KV<String, String>> pubsubMessageErrorCoder =
        FailsafeElementCoder.of(
            PubsubMessageWithAttributesCoder.of(),
            KvCoder.of(StringUtf8Coder.of(), StringUtf8Coder.of()));
    FailsafeElementCoder<String, String> bqFailedInsertRowErrorCoder =
        FailsafeElementCoder.of(StringUtf8Coder.of(), StringUtf8Coder.of());
    CoderRegistry coderRegistry = p.getCoderRegistry();
    coderRegistry.registerCoderForType(
        pubsubMessageErrorCoder.getEncodedTypeDescriptor(), pubsubMessageErrorCoder);
    coderRegistry.registerCoderForType(
        bqFailedInsertRowErrorCoder.getEncodedTypeDescriptor(), bqFailedInsertRowErrorCoder);
    ...
    PCollectionTuple transformOut =
        jsonEvent.apply("ConvertMessageToTableRow", new PubSubMessageToTableRowTransform());

    WriteResult writeResult =
        transformOut
            .get(PubSubMessageToTableRowTransform.TRANSFORM_OUT)
            .apply(
                "Events Write",
    ...
    PCollection<KV<String, TableRow>> failedRows =
        writeResult
            .getFailedInserts()
            .apply(
                "Add Temp Table Id As Key",
                WithKeys.of(
                    row ->
                        String.format("%s%s", row.get("EventName").toString(), TEMP_TABLE_PREFIX)))
            .setCoder(KvCoder.of(StringUtf8Coder.of(), TableRowJsonCoder.of()));
    WriteResult bqSchemaMutationWriteErrors =
        failedRows.apply(
            "Mutation Event Write to Temp Table",
            BigQueryIO.<KV<String, TableRow>>write()
                .to(new BQDestination(options.getDataSetId(), options.getProject()))
                .withFormatFunction(
                    element -> {
                ...
                .withFailedInsertRetryPolicy(InsertRetryPolicy.retryTransientErrors())
                .withCreateDisposition(BigQueryIO.Write.CreateDisposition.CREATE_IF_NEEDED));
    PCollection<FailsafeElement<String, String>> faileSafeWrappedMutationErrors =
        bqSchemaMutationWriteErrors
            .getFailedInserts()
            .apply(
                "WrapInsertionErrors",
                MapElements.into(FAILSAFE_ELEMENT_CODER.getEncodedTypeDescriptor())
                    .via(Util::wrapBigQueryInsertError));
    writeResult
        .getFailedInserts()
        ...
        .apply("Group By Event Type", GroupByKey.create())
        .apply(
            "Update Table Schema",
            ParDo.of(new TableRowSchemaMutator(options.getProject(), options.getDataSetId())));
    transformOut
        .get(PubSubMessageToTableRowTransform.TRANSFORM_DEADLETTER_OUT)
        .apply(
            "Write JSON Error Message",
            WritePubSubErrorMessageTransform.newBuilder()
                ...
                .setErrorRecordsTableSchema(Util.getDeadletterTableSchemaJson())
                .build());
    faileSafeWrappedMutationErrors.apply(
