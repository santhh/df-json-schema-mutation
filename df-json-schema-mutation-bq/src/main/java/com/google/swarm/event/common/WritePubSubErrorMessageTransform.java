...
public abstract class WritePubSubErrorMessageTransform
		extends PTransform<PCollection<FailsafeElement<PubsubMessage, KV<String, String>>>, WriteResult> {
	...
	@Override
	public WriteResult expand(PCollection<FailsafeElement<PubsubMessage, KV<String, String>>> failedRecords) {
		LOG.info("table name {}", getErrorRecordsTable());

		return failedRecords.apply("FailedRecordToTableRow", ParDo.of(new FailedPubsubMessageToTableRowFn())).apply(
	...
	public static class FailedPubsubMessageToTableRowFn
		...
		public void processElement(ProcessContext context) {
			...
			if (message.getPayload() != null) {
				failedRow.set("payloadString", new String(message.getPayload())).set("payloadBytes",
						message.getPayload());
			}
	...
	private static List<TableRow> attributeMapToTableRows(Map<String, String> attributeMap) {
		...
		if (attributeMap != null) {
			attributeMap.forEach(
					(key, value) -> attributeTableRows.add(new TableRow().set("key", key).set("value", value)));
		}
